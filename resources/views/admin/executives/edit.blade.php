...
@section('title', 'Edit Executives')
@section('header', 'Edit Executives')
...
@section('content')
    ...
    <div class="w-full py-22">
        ...
        <form action="{{ route('admin.executives.update', $executive->id) }}" class="border border-slate-700 w-2/3 mx-auto p-4 px-6 rounded-lg" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <label for="name" class="block font-light my-2 text-slate-100">
                ...
                <input type="name" id="name"
                    ...
                    placeholder="Enter name" value="{{ old('name') ?? $executive->name }}" name="name" />
