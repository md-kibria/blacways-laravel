...
@section('content')
    ...
    <div class="w-full py-22">
        ...
        <form action="{{ route('admin.gallery.store') }}" class="border border-slate-700 max-w-lg mx-auto p-4 px-6 rounded-lg" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <label for="category_id" class="block font-light my-2 text-slate-100">
                <span class="text-sm font-medium text-gray-200"> Category </span>
                <select id="category_id"
                    class="mt-0.5 w-full rounded shadow-sm sm:text-sm border @error('category_id') border-red-300 @else border-gray-600 @enderror bg-gray-900 text-white py-2 px-3"
                    name="category_id">
                ...
                @error('category_id') <span class="text-light text-red-300">{{$message}}</span> @enderror
            </label>
