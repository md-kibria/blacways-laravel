...
@section('content')
    ...
    <div class="w-full py-22">
        ...
        <form action="{{ route('admin.news.store') }}" class="border border-slate-700 w-2/3 mx-auto p-4 px-6 rounded-lg" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
            ...
            <label for="content" class="block font-light my-2 text-slate-100">
                ...
                <textarea name="content" id="content" cols="30" rows="10" class="mt-0.5 w-full rounded shadow-sm sm:text-sm border @error('content') border-red-300 @else border-gray-600 @enderror bg-gray-900 text-white py-2 px-3" placeholder="Enter content">{{ old('content') }}</textarea>
