            ...
            <div class="container flex flex-col items-center justify-center relative gap-6 mb-20">
                ...
                <div class="grid grid-cols-2">
                    ...
                    @php
                        ...
                        $googleCalendarUrl .=
                            '&dates=' .
                            now()->parse($event->start_time)->format('Ymd\THis\Z') .
                            '/' .
                            now()->parse($event->end_time)->format('Ymd\THis\Z');
                        $googleCalendarUrl .= '&details=' . urlencode($event->description);
                        $googleCalendarUrl .= '&location=' . urlencode($event->location);
                    @endphp
                    <a href="{{ $googleCalendarUrl }}"
                        class="relative text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 uppercase">
                        Add to Calendar</a>
                </div>
            </div>
        </div>
